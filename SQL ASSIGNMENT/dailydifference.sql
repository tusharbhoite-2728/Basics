...
AS
BEGIN
    SELECT   TOP (SELECT COUNT(*) - 1 FROM pd_1  WHERE DATEPART(HOUR, pd_1.[timestamp]) = 0  AND DATEPART(MINUTE , pd_1.[timestamp]) =0  )
...
WHERE DATEPART(HOUR, pd_1.[timestamp]) = 0  AND DATEPART(MINUTE , pd_1.[timestamp]) =0 
