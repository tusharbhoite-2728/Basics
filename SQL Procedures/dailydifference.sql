...
CREATE OR ALTER PROCEDURE GetPdDailyDeltaMatrix
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Build UNION ALL: (Timestamp, Series, DeltaDay)
    WHILE @i <= 20
    BEGIN
...
' ELSE N'' END
            + N'
SELECT
    CAST(t1.[timestamp] AS DATETIME) AS [Timestamp],   -- removes fractions
    N''' + @table + N''' AS Series,
    (t2.[value] - t1.[value]) AS Delta
FROM ' + QUOTENAME(@table) + N' t1
LEFT JOIN ' + QUOTENAME(@table) + N' t2
    ON t2.[timestamp] = DATEADD(DAY, 1, t1.[timestamp])
WHERE DATEPART(HOUR, t1.[timestamp]) = 0
  AND DATEPART(MINUTE, t1.[timestamp]) = 0
';
        SET @i += 1;
    END;
